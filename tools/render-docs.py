...
SITE_TITLE = "omnibin"
REPO = "https://github.com/fzakaria/omnibin"
# Empty until omnibin has a property of its own. Pointing it at another
# site's id would put this traffic in that site's reporting, so the tag is
# omitted entirely rather than defaulted to a neighbour.
GA_ID = ""
...
def strip_tags(fragment):
    """The visible text of a heading, so `<code>mvs</code>` slugs as `mvs`."""
...
# Lexing the REPL lines as Nix is the bigger win, because they are Nix expressions,
...
# of those subcommands share a name with a bash builtin, such as `nix eval`, `nix
...
def highlight_console(code):
    ...
    for line in code.split("\n"):
        ...
        for pattern, lang in CONSOLE_PROMPTS:
            ...
            # Only the shell half needs it, since a `nix-repl>` line is Nix, where
...
def page_order(docs_dir):
    """The sidebar order, read from the ordered list in index.md, which is one source
...
def provenance(store_path):
    ...
    here, because a docs page runs no JavaScript, so the path is stamped in as a
...
# The analytics tag, or nothing at all when GA_ID is empty.
ANALYTICS = (
...
def shell(title, body, nav, name, store_path, origin):
    ...
    part of omnibin.dev rather than as a separate manual."""
...
def main():
    ...
    # Rendering outside the site derivation, as a local preview does, knows neither
    ...
    # Anything beside the markdown, such as the diagrams a page links to as
